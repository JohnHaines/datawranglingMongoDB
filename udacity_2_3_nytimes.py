"""
From Udacity Data Wrangling With MongoDB  Lesson 2 Excercise 3

This exercise uses:
- codecs module to write unicode files
- authentication with web APIs
- offset when accessing web APIs
...
def get_from_file(kind, period):
    filename = "data/popular.json".format(kind, period)
...
def article_overview(kind, period):
    ...
    urls =[]
    for d in data:
        titles.append({d.get("section"):d.get("title")})
        print (d)
        m = d.get("media")
        print(m)
        for mm in m:
            u = mm.get('media-metadata')
            for i in u:
                if i.get("format") == "Standard Thumbnail":
                    urls.append(i.get("url"))
            print (urls)
    return (titles, urls)

# Alternative
def article_overview_alt(kind, period):
    data = get_from_file(kind, period)
    titles = []
    urls =[]
...
def pretty_print(data, indent=4):
    #After we get our output, we can use this function to format it to be more readable.
...
def main():
    ...
    for k in results["artists"]:
        
        if k.get("name") == "Nirvana" and k.get("country") == "US":
            artist_id = k["id"]
